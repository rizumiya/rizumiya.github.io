...
window.setGlobalMute = function (isMuted) {
    if (bgAudio) bgAudio.muted = isMuted;
    if (focusAudio) focusAudio.muted = isMuted;
    if (pressAudio) pressAudio.muted = isMuted;
    // Opsional: Matikan suara efek lain jika ada
    if (typeof ee !== 'undefined') ee.muted = isMuted; // Suara myarea
    if (typeof te !== 'undefined') te.muted = isMuted; // Suara missile
    if (typeof ae !== 'undefined') ae.muted = isMuted; // Suara gameover
    if (typeof ne !== 'undefined') ne.muted = isMuted; // Suara spawn
    if (typeof re !== 'undefined') re.muted = isMuted; // Suara gamewin
};
